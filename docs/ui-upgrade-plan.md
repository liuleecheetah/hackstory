...
**狀態：** 已定案，待開發（U0 → U1 → U2 → U2.5 → U3 依序執行；U4 暫停）
**最後更新：** 2026-09-17
...
> **本文件裡標「決策點」的地方，動手前要先問使用者。** 其餘照做，不要自行擴大範圍。
...
- **色盲安全色盤**（給圖層與軸線；住在 U1 的 `render/theme.ts`，U0 先不動 `PALETTE`）：以 Okabe–Ito 為基礎的六色
  ...
  **注意：換色盤會改變預設軸線顏色，使用者會看到範例的顏色變了，U1 開工前要先講。**
...
**決策點：** 兩種粗細的切片加起來約 6–8 MB 進 repo。動手前告訴使用者這個數字，確認可以。
...
| 步驟 | 要問什麼 | 我的建議 |
|---|---|---|
| U0a | 字型切片約 6–8 MB 進 repo，可以嗎？ | 可以；GitHub Pages 不在乎，讀者只下載用到的切片 |
| U1 | 換色盲安全色盤會改變範例的預設顏色 | 換，並讓使用者看前後對照 |
| U2 | QR code 需要一個產生器。選項：安裝 `qrcode` 套件（純 JS、無相依、約 30 KB）或自己寫（約 400 行、要測試） | 安裝 `qrcode`。CLAUDE.md 規定裝東西要先問 |
| U2 | 「複製到剪貼簿」在 Safari 需要使用者手勢內完成，行為可能與 Chrome 不同 | 做，但 Safari 失敗時退回「已下載」 |
| U2.5 | SPEC 升 0.5 的欄位設計（2.4.1）是否同意 | 同意再動 core |
